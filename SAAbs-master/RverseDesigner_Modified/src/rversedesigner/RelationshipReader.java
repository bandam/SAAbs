...
public class RelationshipReader {
    ...
    /**
     * Concern Detector Methods
     */
    public String getConcernDetectorRelationshipMatrices() {
        String relationshipUML = "";

        try {
            relationshipUML += readConcernDetectorAssociation(csvAssociation);
        } catch (FileNotFoundException ex) {
            Logger.getLogger(ReverseDesignerApp.class.getName()).log(Level.SEVERE, null, ex);
        } catch (IOException ex) {
            Logger.getLogger(ReverseDesignerApp.class.getName()).log(Level.SEVERE, null, ex);
        ...
        try {
            relationshipUML += readConcernDetectorGeneralization(csvGeneralization);
        } catch (FileNotFoundException ex) {
            Logger.getLogger(ReverseDesignerApp.class.getName()).log(Level.SEVERE, null, ex);
        } catch (IOException ex) {
            Logger.getLogger(ReverseDesignerApp.class.getName()).log(Level.SEVERE, null, ex);
    ...
    public String readConcernDetectorAssociation(String csvFile) throws FileNotFoundException, IOException {
        ArrayList<com.amirab.roleDetector.Class> concernDetectorClasses = getConcernDetectorClasses();
        String[] row = null;
        String[] topRow = null;
        List<String> assocList;
        assocList = new ArrayList<>();
        ...
        while ((row = csvReader.readNext()) != null) {
            ...
            while (count < row.length) {
                if (Integer.parseInt(row[count]) > 0) {
                    String inList = null;
                    inList = getBiggestConcern(concernDetectorClasses, row[0]) + row[0] + " --> " + getBiggestConcern(concernDetectorClasses, topRow[count]) + topRow[count];
                    inListInvert = getBiggestConcern(concernDetectorClasses, topRow[count]) + topRow[count] + " --> " + getBiggestConcern(concernDetectorClasses, row[0]) + row[0];

                    if (assocList.size() != 0) {
                        int listLoop = 0;
                        for (listLoop = 0; listLoop < assocList.size(); listLoop++) {
                            if (assocList.get(listLoop).equalsIgnoreCase(inListInvert)) {
                                newList = getBiggestConcern(concernDetectorClasses, row[0]) + row[0] + " -- " + getBiggestConcern(concernDetectorClasses, topRow[count]) + topRow[count];
                                assocList.set(listLoop, newList);
                                break;
                            } else {
                                newList = null;
    ...
    public String readConcernDetectorGeneralization(String csvFileGen) throws FileNotFoundException, IOException {
        ArrayList<com.amirab.roleDetector.Class> concernDetectorClasses = getConcernDetectorClasses();
        ...
        while ((row = csvReader.readNext()) != null) {
            ...
            while (count < row.length) {
                if (Integer.parseInt(row[count]) > 0) {
                    plantUMLScript += getBiggestConcern(concernDetectorClasses, row[0]) + row[0] + " --|> " + getBiggestConcern(concernDetectorClasses, topRow[count]) + topRow[count] + "\n";
                }
                count++;
            }
        }
    ...
    public ArrayList<com.amirab.roleDetector.Class> getConcernDetectorClasses() {
    ...
    private String getBiggestConcern(ArrayList<com.amirab.roleDetector.Class> concernClass, String name) {
        for (com.amirab.roleDetector.Class c : concernClass) {
            if (c.getName().equals(name)) {
                return c.getBiggestConcernSterotype() + ".";
            }
        }
