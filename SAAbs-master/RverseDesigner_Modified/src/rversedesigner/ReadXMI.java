...
//    }
        ...
        for (cntClsList = 0; cntClsList < classList.size(); cntClsList++) {
            ...
            ColorAdjuster changeColor = new ColorAdjuster();
            currColor = changeColor.colorCalculator(currColor, darker);
            plantUMLXMI += "class " + classList.get(cntClsList).replaceAll("//", "").replace("$", "").replaceAll("/", "_") + " \n";
            plantUMLClass += "class " + classList.get(cntClsList) + " \n";
        }
     ...
     */
    public String readConcernDetectorClassDiagram() {
        ...
        for (cntClsList = 0; cntClsList < classList.size(); cntClsList++) {
            Class concernClass = findConcernClass(concernDetectorClasses, classList.get(cntClsList));
            //String source = "@startuml\n";
            ColorAdjuster changeColor = new ColorAdjuster();
            currColor = changeColor.colorCalculator(currColor, darker);

            if (concernClass != null) {
                //String biggestConcernName = concernClass.getBiggestConcernSterotype().replace("\\s", "_");
                plantUMLXMI += "class " + classList.get(cntClsList) + concernClass.getConcernStereotypeList() + " \n";
                plantUMLClass += "class " + classList.get(cntClsList) + concernClass.getConcernStereotypeList() + " \n";
                plantUMLGetSetOut += "class " + classList.get(cntClsList) + concernClass.getConcernStereotypeList() + " \n";
            } else {
                plantUMLXMI += "class " + classList.get(cntClsList) + " \n";
                plantUMLClass += "class " + classList.get(cntClsList) + " \n";
                plantUMLGetSetOut += "class " + classList.get(cntClsList) + " \n";
                    ...
                    for (int i = 0; i < classList.size(); i++) {
                        ...
                        if (concernClass != null) {
                            ...
                            if (classList.get(i).equalsIgnoreCase(me.getOwner().getName())) {
                                ...
                                plantUMLXMI += me.getName() + "()\n";
                                plantUMLXMI += "}\n";
                                plantUMLOperation += "class " + biggestConcernName + "." + me.getOwner().getName() + "{\n";
                                plantUMLOperation += me.getName() + "()\n";
                                plantUMLOperation += "}\n";
                                if (me.getName().startsWith("get") || me.getName().endsWith("get")
                                        || me.getName().startsWith("set") || me.getName().endsWith("set") || me.getName().startsWith("to")
                                    ...
                                    plantUMLGetSetOut += "class " + biggestConcernName + "." + me.getOwner().getName() + "{\n";
                                    plantUMLGetSetOut += me.getName() + "()\n";
                                    plantUMLGetSetOut += "}\n";
                                    System.out.println(me.getOwner().getName() + " : " + me.getName() + " : getters/setters found");
                                }
                    ...
                    for (int i = 0; i < classList.size(); i++) {
                        ...
                        if (concernClass != null) {
                            ...
                            if (classList.get(i).equalsIgnoreCase(me.getOwner().getName())) {
                                ...
                                plantUMLXMI += me.getName() + "\n";
                                plantUMLXMI += "}\n";
                                plantUMLAttribute += "class " + biggestConcernName + "." + me.getOwner().getName() + "{\n";
                                plantUMLAttribute += me.getName() + "\n";
                                plantUMLAttribute += "}\n";
                                plantUMLGetSetOut += "class " + biggestConcernName + "." + me.getOwner().getName() + "{\n";
                                plantUMLGetSetOut += me.getName() + "\n";
    ...
    private Class findConcernClass(ArrayList<Class> concernClass, String name) {
        for (Class c : concernClass) {
